...
namespace TiaMcpServer.ModelContextProtocol
{
    ...
    public static partial class McpServer
    {
            ...
            [Description("typePath: defines the path in the project structure to the type")] string typePath)
        {
            ...
            {
                throw McpError.WithRecovery(ex, $"Unexpected error retrieving type info from '{typePath}' in '{softwarePath}': {ex.Message}{McpHints.Recovery(ex)}");
            }
        }
            ...
            [Description("regexName: defines the name or regular expression to find the block. Use empty string (default) to find all")] string regexName = "")
        {
            ...
            {
                throw McpError.WithRecovery(ex, $"Unexpected error retrieving user defined types with regex '{regexName}' in '{softwarePath}': {ex.Message}{McpHints.Recovery(ex)}");
            }
        }
            ...
            [Description("preservePath: preserves the path/structure of the plc software")] bool preservePath = false)
        {
            ...
            {
                throw McpError.WithRecovery(ex, $"Unexpected error exporting type from '{typePath}' to '{exportPath}': {ex.Message}{McpHints.Recovery(ex)}");
            }
        }
            ...
            [Description("preservePath: keep hierarchy in temp dir")] bool preservePath = false)
        {
            ...
            {
                throw McpError.WithRecovery(ex, $"Unexpected error exporting type to temp: {ex.Message}{McpHints.Recovery(ex)}");
            }
        }
            ...
            [Description("importPath: defines the path of the xml file from where to import the type")] string importPath)
        {
            ...
            {
                throw McpError.WithRecovery(pex, $"Failed importing type from '{importPath}' to '{groupPath}' [{pex.Code}]: {pex.Message}");
            }
            catch (Exception ex) when (ex is not McpException)
            {
                throw McpError.WithRecovery(ex, $"Unexpected error importing type from '{importPath}' to '{groupPath}': {ex.Message}{McpHints.Recovery(ex)}");
            }
        }
            ...
            [Description("placeholders: JsonObject key-values for replacement in XML, e.g. {\"PLC_NAME\":\"PLC_1\"}")] JsonObject? placeholders = null)
        {
            ...
            {
                throw McpError.WithRecovery(ex, $"Unexpected error seeding project from reference: {ex.Message}{McpHints.Recovery(ex)}");
            }
        }
            ...
            [Description("preservePath: preserves the path/structure of the plc software")] bool preservePath = false)
        {
            ...
            catch (Exception ex) when (ex is not McpException)
            {
                ...
                throw McpError.WithRecovery(ex, $"Unexpected error exporting types '{regexName}' from '{softwarePath}' to {exportPath}: {ex.Message}{McpHints.Recovery(ex)}");
            }
        }
            ...
            [Description("preservePath: keep hierarchy in temp dir")] bool preservePath = false)
        {
            ...
            {
                throw McpError.WithRecovery(ex, $"Unexpected error exporting types to temp: {ex.Message}{McpHints.Recovery(ex)}");
            }
        }
