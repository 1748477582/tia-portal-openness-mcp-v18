...
namespace TiaMcpServer.ModelContextProtocol
{
    ...
    public static partial class McpServer
    {
        ...
        public static ResponseProjectTree GetProjectTree()
        {
            ...
            {
                throw McpError.WithRecovery(ex, $"Unexpected error retrieving project tree: {ex.Message}{McpHints.Recovery(ex)}");
            }
        }
            ...
            [Description("devicePath: defines the path in the project structure to the device")] string devicePath)
        {
            ...
            {
                throw McpError.WithRecovery(ex, $"Unexpected error retrieving device info from '{devicePath}': {ex.Message}{McpHints.Recovery(ex)}");
            }
        }
            ...
            [Description("maxDepth: recursion depth, default 4")] int maxDepth = 4)
        {
            ...
            {
                throw McpError.WithRecovery(ex, $"Unexpected error retrieving device item tree from '{deviceItemPath}': {ex.Message}{McpHints.Recovery(ex)}");
            }
        }
            ...
            [Description("deviceItemPath: path in the project structure to the device item")] string deviceItemPath)
        {
            ...
            {
                throw McpError.WithRecovery(ex, $"Unexpected error retrieving network info from '{deviceItemPath}': {ex.Message}{McpHints.Recovery(ex)}");
            }
        }
            ...
            [Description("exportPath: target folder or full .aml file path")] string exportPath)
        {
            ...
            {
                throw McpError.WithRecovery(pex, $"CAx/AML export failed for '{devicePath}': {pex.Message}");
            }
            catch (Exception ex) when (ex is not McpException)
            {
                throw McpError.WithRecovery(ex, $"Unexpected error exporting AML from '{devicePath}': {ex.Message}");
            }
        }
            ...
            [Description("value: new value as string; converted based on current attribute type")] string value)
        {
            ...
            {
                throw McpError.WithRecovery(ex, $"Unexpected error setting device item attribute '{attributeName}': {ex.Message}{McpHints.Recovery(ex)}");
            }
        }
            ...
            [Description("expectedHmiSoftwarePath: expected HMI software path, empty to skip")] string expectedHmiSoftwarePath = "HMI_RT_1")
        {
            ...
            {
                throw McpError.WithRecovery(ex, $"Unexpected error validating automation context: {ex.Message}{McpHints.Recovery(ex)}");
            }
        }
            ...
            [Description("subnetName: subnet name to create when the first node is not already connected, e.g. 'PN_IE_1'")] string subnetName = "PN_IE_1")
        {
            ...
            {
                throw McpError.WithRecovery(ex, $"Unexpected error connecting device nodes to PROFINET subnet: {ex.Message}{McpHints.Recovery(ex)}");
            }
        }
            ...
            [Description("planJson: JSON with operations[]. Supported operation types: EnsureSubnet, AttachDeviceNodeToSubnet, SetCpuCommonSettings. This is offline-only and performs validation only.")] string planJson)
        {
            ...
            {
                throw McpError.WithRecovery(ex, $"Unexpected error validating hardware network plan: {ex.Message}{McpHints.Recovery(ex)}");
            }
        }
            ...
            [Description("subnetName: exact subnet name to create or read back, e.g. PN_IE_1.")] string subnetName)
        {
            ...
            {
                throw McpError.WithRecovery(ex, $"Unexpected error ensuring subnet '{subnetName}': {ex.Message}{McpHints.Recovery(ex)}");
            }
        }
            ...
            [Description("anchorDeviceItemPath: optional real device-item path used to EnsureSubnet first when subnetName is not found.")] string anchorDeviceItemPath = "")
        {
            ...
            {
                throw McpError.WithRecovery(ex, $"Unexpected error attaching device node to subnet '{subnetName}': {ex.Message}{McpHints.Recovery(ex)}");
            }
        }
            ...
            [Description("settingsJson: JSON object { \"exactAttributes\": { \"ExactAttributeNameFromReadback\": \"value\" } }. No aliases or guessed attribute names are accepted.")] string settingsJson)
        {
            ...
            {
                throw McpError.WithRecovery(ex, $"Unexpected error setting CPU common settings for '{cpuPath}': {ex.Message}{McpHints.Recovery(ex)}");
            }
        }
            ...
            [Description("deepScan: when true include ancestor/project/device-level candidates; when false only direct node/interface/device-item candidates")] bool deepScan = true)
        {
            ...
            {
                throw McpError.WithRecovery(ex, $"Unexpected error probing hardware HMI connection owner candidates: {ex.Message}{McpHints.Recovery(ex)}");
            }
        }
            ...
            [Description("deepScan: when true include safe ancestors; when false only direct node/interface/device-item candidates")] bool deepScan = true)
        {
            ...
            {
                throw McpError.WithRecovery(ex, $"Unexpected error scanning hardware HMI connection whitelisted services: {ex.Message}{McpHints.Recovery(ex)}");
            }
        }
        ...
        public static ResponseDevices GetDevices()
        {
            ...
            {
                throw McpError.WithRecovery(ex, $"Unexpected error retrieving devices: {ex.Message}{McpHints.Recovery(ex)}");
            }
        }
            ...
            [Description("deviceName: name in project tree")] string deviceName)
        {
            ...
            {
                throw McpError.WithRecovery(pex, $"Failed to add device '{deviceName}' ({orderNumber} {version}) [{pex.Code}]: {pex.Message}");
            }
            catch (Exception ex) when (ex is not McpException)
            {
                throw McpError.WithRecovery(ex, $"Unexpected error adding device: {ex.Message}{McpHints.Recovery(ex)}");
            }
        }
            ...
            [Description("family: fallback hint, e.g. 'S7-1200', 'S7-1500', or 'WinCCUnifiedPC'")] string family = "S7-1500")
        {
            ...
            {
                throw McpError.WithRecovery(ex, $"Unexpected error adding device with fallback: {ex.Message}{McpHints.Recovery(ex)}");
            }
        }
            ...
            [Description("limit: maximum candidates to return")] int limit = 50)
        {
            ...
            {
                throw McpError.WithRecovery(ex, $"Unexpected error searching installed GSD devices: {ex.Message}{McpHints.Recovery(ex)}");
            }
        }
            ...
            [Description("limit: maximum candidates to return")] int limit = 50)
        {
            ...
            {
                throw McpError.WithRecovery(pex, $"Failed searching hardware catalog for '{keyword}' [{pex.Code}]: {pex.Message}");
            }
            catch (Exception ex) when (ex is not McpException)
            {
                throw McpError.WithRecovery(ex, $"Unexpected error searching hardware catalog: {ex.Message}{McpHints.Recovery(ex)}");
            }
        }
            ...
            [Description("preferredDap: optional DAP/version hint, e.g. 'DAP V2.0', 'DAP 1', or empty")] string preferredDap = "")
        {
            ...
            {
                throw McpError.WithRecovery(ex, $"Unexpected error adding GSD device with probe: {ex.Message}{McpHints.Recovery(ex)}");
            }
        }
            ...
            [Description("preferredText: optional ranking hint, e.g. 'PN 17.0.0.0 landscape', '6AV2 123-2GB03-0AX0'")] string preferredText = "")
        {
            ...
            {
                throw McpError.WithRecovery(ex, $"Unexpected error adding hardware catalog device with probe: {ex.Message}{McpHints.Recovery(ex)}");
            }
        }
