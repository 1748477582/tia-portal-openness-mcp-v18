...
namespace TiaMcpServer.ModelContextProtocol
{
    ...
    public static partial class McpServer
    {
            ...
            [Description("blockPath: defines the path in the project structure to the block")] string blockPath)
        {
            ...
            {
throw McpError.WithRecovery(ex, $"Unexpected error retrieving block info from '{blockPath}' in '{softwarePath}': {ex.Message}{McpHints.Recovery(ex)}");
            }
        }
            ...
            [Description("regexName: defines the name or regular expression to find the block. Use empty string (default) to find all")] string regexName = "")
        {
            ...
            {
throw McpError.WithRecovery(ex, $"Unexpected error retrieving blocks with regex '{regexName}' in '{softwarePath}': {ex.Message}{McpHints.Recovery(ex)}");
            }
        }

        [McpServerTool(Name = "GetBlocksWithHierarchy"), Description("[L2][PLC-Software]Get a list of all blocks with their group hierarchy from the plc software. AI workflow: call this (or the compact GetProjectSkeleton) FIRST to learn structure & used numbers before creating/editing blocks; then SuggestBlockNumber before importing; then AnalyzeBlockImpact before changing a block.")]
...
throw McpError.WithRecovery(ex, $"Unexpected error retrieving block hierarchy for '{softwarePath}': {ex.Message}{McpHints.Recovery(ex)}");
            }
        }



        [McpServerTool(Name = "ExportBlock"), Description("[L2][PLC-Software] Export one block to a SimaticML XML file (NOT .s7dcl text — that needs V20+, which is disabled in this V18 build). Requires: Connect + OpenProject + block must be consistent (compile first if IsConsistent=false). blockPath must be fully qualified 'Group/Subgroup/Name' from GetSoftwareTree — bare names return InvalidParams with suggestions. Pick the right tool: batch → ExportBlocks; readable LOGIC → DescribeBlockLogic; editable export to MODIFY the block → ExportBlockSourceUtf8.")]
            ...
            [Description("preservePath: preserves the path/structure of the plc software")] bool preservePath = false)
        {
            ...
            {
throw McpError.WithRecovery(ex, $"Unexpected error exporting block from '{blockPath}' to '{exportPath}': {ex.Message}{McpHints.Recovery(ex)}");
            }
        }
            ...
            [Description("preservePath: keep hierarchy in temp dir")] bool preservePath = false)
        {
            ...
            {
throw McpError.WithRecovery(ex, $"Unexpected error exporting block to temp: {ex.Message}{McpHints.Recovery(ex)}");
            }
        }
        ...
        public static ResponseImportBlock ImportBlock(
            ...
            [Description("preferredNumber: if set, renumber the imported block to this number (overrides autoAssignNumber)")] int? preferredNumber = null,
            [Description("autoAssignNumber: if true, assign a free block number in the same bucket automatically")] bool autoAssignNumber = false)
        {
            try
            {
                ...
                // F2: optional auto / preferred block numbering right after import.
                int? assignedNumber = null; bool renumbered = false; string numberNote = "";
                if ((preferredNumber.HasValue || autoAssignNumber) && found != null && found.Count > 0)
                {
                    try
                    {
                ...
                {
                    Message = $"Block imported from '{importPath}' to '{groupPath}'" + (verified ? " (verified)" : "") + (renumbered ? $" (auto-renumbered to {assignedNumber})" : ""),
                    Meta = new JsonObject
                    {
                        ["timestamp"] = DateTime.Now,
                        ["success"] = true,
                        ...
                        ["assignedNumber"] = JsonValue.Create(assignedNumber),
                        ["renumbered"] = renumbered,
                        ["autoNumberNote"] = numberNote
                    }
                };
            }
            ...
            {
throw McpError.WithRecovery(ex, $"Failed importing block from '{importPath}' to '{groupPath}': {ex.Message}{McpHints.Recovery(ex)}");
            }
        }

        #region F2 auto-numbering helpers (distinct names to avoid clashing with Optimizations.cs BucketType/SafeBlockNumber)
        private static string BlockBucketType(string typeName)
        {
            // Must stay consistent with DetectBlockSubtype (Portal.Helpers.cs); the previous
            // strict "FunctionBlock"/"DataBlock"/"OrganizationBlock" string checks mis-classified
            // blocks whose runtime type name is the short form (e.g. "FB") as "UDT", which made the
            // F2 auto-renumber guard `if (bucket != "UDT")` skip renumbering entirely.
            // net48-compatible: only string.Contains(string) / string.EndsWith(string) overloads.
            var t = typeName ?? string.Empty;
            if (t.Contains("FunctionBlock") || t.EndsWith("FB")) return "FB";
            if (t.Contains("Function") || t.EndsWith("FC")) return "FC";
            if (t.Contains("DataBlock") || t.EndsWith("DB")) return "DB";
            if (t.Contains("OrganizationBlock") || t.EndsWith("OB")) return "OB";
            return "UDT";
        }

        private static int FreeBlockNumber(string softwarePath, string bucket, int? preferredNumber)
        {
            // TIA Portal enforces GLOBAL block-number uniqueness (an FB and a DB cannot share a
            // number) — proven by SetBlockNumber's global collision pre-check in Portal.Helpers.cs.
            // So we MUST scan ALL buckets for used numbers, not just the target `bucket`; otherwise
            // we would return a number that is free within the bucket (e.g. 4 in the FB bucket) but
            // already taken globally (e.g. by DB4), and SetBlockNumber would then fail with
            // "number N is already used by block '<other>'".
            var blocks = Portal.GetBlocks(softwarePath, "");
            ...
            [Description("overwrite: true=Override, false=Rename")] bool overwrite = true)
        {
            ...
            {
throw McpError.WithRecovery(ex, $"Unexpected error importing blocks from '{dir}' to '{groupPath}': {ex.Message}{McpHints.Recovery(ex)}");
            }
        }
...
throw McpError.WithRecovery(ex, $"Unexpected error compiling software '{softwarePath}': {ex.Message}{McpHints.Recovery(ex)}");
            ...
            [Description("compileAfter: compile PLC after import")] bool compileAfter = true)
        {
            ...
            {
throw McpError.WithRecovery(ex, $"Unexpected error repairing/reimporting block '{importPath}': {ex.Message}{McpHints.Recovery(ex)}");
            }
        }
        ...
        [McpServerTool(Name = "ExportBlocks"), Description("[L2][PLC-Software] Export all (or regexName-filtered) blocks to a directory as SimaticML XML (NOT .s7dcl text — that needs V20+, disabled in this V18 build). Pick the right tool: a single block → ExportBlock; editable export to MODIFY a block → ExportBlockSourceUtf8; readable LOGIC of one block → DescribeBlockLogic.")]
...
throw McpError.WithRecovery(ex, $"Unexpected error exporting blocks with '{regexName}' from '{softwarePath}' to {exportPath}: {ex.Message}{McpHints.Recovery(ex)}");
            }
        }
            ...
            [Description("preservePath: keep hierarchy in temp dir")] bool preservePath = false)
        {
            ...
            {
throw McpError.WithRecovery(ex, $"Unexpected error exporting blocks to temp: {ex.Message}{McpHints.Recovery(ex)}");
            }
        }

        [McpServerTool(Name = "AnalyzeBlockImpact"), Description("[L2][PLC-Software] IMPACT ANALYSIS (global view). Given one block, reports (1) its interface pins (Input/Output/Static/Constant/Return with datatypes) and (2) which other blocks CALL it, by scanning all in-scope code blocks' exported SimaticML for <Call> references. Openness has no reverse call-index API, so this reuses the Export+XML-parse approach proven by TraceTagCause. ALWAYS run this BEFORE editing a block so you see the whole blast radius, not just the single exported file. For a forward call graph (which blocks a block CALLs), use GetCallChain direction=callees. Requires: Connect + OpenProject in OFFLINE mode (Openness cannot export while TIA is online). blockScope regex limits which blocks are scanned (default empty = scan all code blocks).")]
        public static ResponseJsonReport AnalyzeBlockImpact(
            [Description("softwarePath: path in project structure to the plc software")] string softwarePath,
            [Description("blockName: name (or Group/BlockName) of the block to analyze")] string blockName,
            [Description("blockScope: optional regex limiting which blocks are scanned for callers (default empty = all code blocks)")] string blockScope = "")
        {
            try
            {
                return Portal.AnalyzeBlockImpact(softwarePath, blockName, blockScope ?? "");
            }
            catch (Exception ex)
            {
throw McpError.WithRecovery(ex, $"Unexpected error analyzing block impact for '{blockName}' in '{softwarePath}': {ex.Message}");
            }
        }
        ...
        public static ResponseExportBlock ExportBlockSourceUtf8(
            ...
            [Description("exportPath: directory to write the .xml into")] string exportPath,
            [Description("preservePath: mirror the block's group hierarchy inside exportPath")] bool preservePath = false)
        {
            try
            {
                var finalPath = Portal.ExportBlockSourceUtf8(softwarePath, blockPath, exportPath, preservePath);
                return new ResponseExportBlock
                {
                    Message = $"Block source exported (UTF-8+BOM) to '{finalPath}'. Edit it, then call RegenerateBlockFromSource to re-import.",
                    Meta = new JsonObject { ["timestamp"] = DateTime.Now, ["success"] = true, ["exportedPath"] = finalPath }
                };
            }
            catch (TiaMcpServer.Siemens.PortalException pex)
            {
                var msg = pex.Code == TiaMcpServer.Siemens.PortalErrorCode.NotFound
                    ? ("Block not found." + BuildBlockDidYouMean(softwarePath, blockPath)).Trim()
                    : $"Export failed: {pex.Message}";
                throw new McpException(msg, pex.Code == TiaMcpServer.Siemens.PortalErrorCode.NotFound ? McpErrorCode.InvalidParams : McpErrorCode.InternalError);
            }
            catch (Exception ex) when (ex is not McpException)
            {
throw McpError.WithRecovery(ex, $"Unexpected error exporting block source from '{blockPath}': {ex.Message}");
            }
        }
        ...
        public static ResponseMessage RegenerateBlockFromSource(
            ...
            [Description("sourceFilePath: path to the edited .xml/.scl/.s7dcl file")] string sourceFilePath)
        {
            try
            {
                var ok = Portal.RegenerateBlockFromSource(softwarePath, groupPath, sourceFilePath);
                return new ResponseMessage
                {
                    Message = ok
                        ? $"Block regenerated from '{sourceFilePath}'. Run CompileAndDiagnosePlc to verify."
                        : $"Regenerate reported no change for '{sourceFilePath}' (block may already match). Run CompileAndDiagnosePlc to verify.",
                    Meta = new JsonObject { ["timestamp"] = DateTime.Now, ["success"] = ok }
                };
            }
            catch (TiaMcpServer.Siemens.PortalException pex)
            {
                throw new McpException($"Regenerate failed: {pex.Message}", pex.Code == TiaMcpServer.Siemens.PortalErrorCode.InvalidParams ? McpErrorCode.InvalidParams : McpErrorCode.InternalError);
            }
            catch (Exception ex) when (ex is not McpException)
            {
throw McpError.WithRecovery(ex, $"Unexpected error regenerating block from '{sourceFilePath}': {ex.Message}");
            }
        }
