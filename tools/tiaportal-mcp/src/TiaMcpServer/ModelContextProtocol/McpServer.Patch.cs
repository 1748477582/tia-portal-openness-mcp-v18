...
namespace TiaMcpServer.ModelContextProtocol
{
    ...
    public static partial class McpServer
    {
        public static ResponseScaffold PatchProject(string spec, bool dryRun = false, bool noOverwrite = false)
        {
            ...
            catch (Exception ex) { Step("connect", "failed", ex.Message); resp.Ok = false; throw McpError.WithRecovery(ex, $"PatchProject aborted at connect: {ex.Message}{McpHints.Recovery(ex)}"); }

            try { OpenProject(projectPath); Step("openProject", "ok", projectPath); }
            catch (Exception ex) { Step("openProject", "failed", ex.Message); resp.Ok = false; throw McpError.WithRecovery(ex, $"PatchProject aborted at openProject: {ex.Message}{McpHints.Recovery(ex)}"); }
