...
namespace TiaMcpServer.ModelContextProtocol
{
    ...
    public static partial class McpServer
    {
        ...
        public static ResponseGetProjects GetProjects()
        {
            ...
            {
                throw McpError.WithRecovery(ex, $"Unexpected error retrieving open projects: {ex.Message}{McpHints.Recovery(ex)}");
            }
        }
            ...
            [Description("path: defines the path where to the project/session")] string path)
        {
            ...
            {
                throw McpError.WithRecovery(ex, $"Unexpected error opening project '{path}': {ex.Message}{McpHints.Recovery(ex)}");
            }
        }
            ...
            [Description("projectName: name shown in TIA (e.g. '项目1')")] string projectName)
        {
            ...
            {
                throw McpError.WithRecovery(ex, $"Unexpected error attaching to open project '{projectName}': {ex.Message}{McpHints.Recovery(ex)}");
            }
        }
            ...
            [Description("projectName: project name")] string projectName)
        {
            ...
            {
                throw McpError.WithRecovery(ex, $"Unexpected error creating project: {ex.Message}{McpHints.Recovery(ex)}");
            }
        }
            ...
            [Description("dryRun: DEFAULT true — validates the spec offline (no TIA connection, nothing created). Pass dryRun=false explicitly to actually create the project (after a clean dry run).")] bool dryRun = true)
        {
            ...
            catch (Exception ex) { Step("connect", "failed", ex.Message); resp.Ok = false; throw McpError.WithRecovery(ex, $"ScaffoldProject aborted at connect: {ex.Message}{McpHints.Recovery(ex)}"); }

            try { CreateProject(directoryPath, projectName); Step("createProject", "ok", directoryPath); }
            catch (Exception ex) { Step("createProject", "failed", ex.Message); resp.Ok = false; throw McpError.WithRecovery(ex, $"ScaffoldProject aborted at createProject: {ex.Message}{McpHints.Recovery(ex)}"); }
            ...
            catch (Exception ex) { Step("addDevicePlc", "failed", ex.Message); resp.Ok = false; throw McpError.WithRecovery(ex, $"ScaffoldProject aborted at addDevicePlc: {ex.Message}{McpHints.Recovery(ex)}"); }
...
#else
                    Step("hmiUnified", "skipped", "WinCC Unified HMI not supported on TIA Portal V18; use Classic HMI tools.");
#endif
            // ---- save ----
            if (B("save", true))
            {
        ...
        public static ResponseSaveProject SaveProject()
        {
            ...
            {
                throw McpError.WithRecovery(ex, $"Unexpected error saving local project/session: {ex.Message}{McpHints.Recovery(ex)}");
            }
        }
            ...
            [Description("newProjectPath: defines the new path where to save the project")] string newProjectPath)
        {
            ...
            {
                throw McpError.WithRecovery(ex, $"Unexpected error saving local project/session as '{newProjectPath}': {ex.Message}{McpHints.Recovery(ex)}");
            }
        }
        ...
        public static ResponseCloseProject CloseProject()
        {
            ...
            {
                throw McpError.WithRecovery(ex, $"Unexpected error closing local project/session: {ex.Message}{McpHints.Recovery(ex)}");
            }
        }
