...
namespace TiaMcpServer.Siemens
{
    ...
    public partial class Portal
    {
        ...
        private static object? TryFindByNameInCollection(object root, string[] propertyHints, string wantedName)
        {
            try
            {
                ...
                foreach (var propName in propertyHints)
                {
                    ...
                    else if (collection != null && propName.EndsWith("Folder", StringComparison.OrdinalIgnoreCase))
                    {
                        // Folder-like object (e.g. ScreenFolder/TagFolder): search its item
                        // collection AND recurse into nested user sub-folders ("Folders").
                        // Classic HMI projects commonly keep every screen inside screen groups,
                        // so a root-only lookup would miss all of them.
                        var itemsPropName = propName.Substring(0, propName.Length - "Folder".Length) + "s";
                        var found = FindInFolderTreeRecursive(collection, itemsPropName, wantedName, 0);
                        if (found != null) return found;
                    }
                }
            }
        ...
        private static object? FindInFolderTreeRecursive(object folder, string itemsPropName, string wantedName, int depth)
        {
            if (folder == null || depth > 12) return null;
            try
            {
                var t = folder.GetType();

                // 1) direct items in this folder (e.g. Screens / TagTables)
                foreach (var cp in new[] { itemsPropName, "Screens" }.Distinct())
                {
                    if (string.IsNullOrEmpty(cp)) continue;
                    var coll = t.GetProperty(cp)?.GetValue(folder);
                    if (coll is System.Collections.IEnumerable en && coll is not string)
                    {
                        foreach (var item in en)
                        {
                            if (item == null) continue;
                ...
                // 2) recurse into nested user folders
                var folders = t.GetProperty("Folders")?.GetValue(folder);
                if (folders is System.Collections.IEnumerable fen)
                {
                    foreach (var sub in fen)
                    {
                        if (sub == null) continue;
                        var found = FindInFolderTreeRecursive(sub, itemsPropName, wantedName, depth + 1);
                        if (found != null) return found;
                    }
                }
            }
