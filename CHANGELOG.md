...
- **新工具 `LintPlcSclSource(sclContent)`［L1，离线］**：对任意 SCL 源文本做静态 lint，返回带源码行号的发现（Meta.lint），非阻断、只读。
- **`WritePlcSclSourceFile` 内建预检**：写 .scl 文件后自动跑 lint，结果附到返回 Meta.lint + Message 摘要（不阻断写入）。
- **`SclLinter` 静态检查类（纯文本解析，无 TIA 依赖）**覆盖 4 类常见问题：
  - SCL001 同名实参：一个块调用里同一形参赋多次（如 `IN1 := 1, IN1 := 2`）。
  - SCL002 错误 DB 访问语法：`"DB".DB(...)` 非法，应为 `"DB_Name".TagName`。
  - SCL003 OB 内声明 FB 多重实例：多重实例必须放在 FB 里，不能放 OB 的 VAR。
  - SCL004 段位误用：`HW_*`/`PORT`/`CONN_*` 等硬件接口类型声明在 VAR_TEMP（必须放 VAR）。
- 验证：net8 独立单元测试 7 例全过（4 类各命中正确行号 + 2 个负例静默 + 嵌套调用不误报）；V18 构建 0 错，已 promote 到运行中的 bin-v18。

## [2.3.0] - 2026-07-04 - 新工具 DescribeBlockLogic：把梯形图读成可读逻辑（含"恒断/禁用行"自动标注）

新增一个把 LAD 网络渲染为可读能流表达式的工具，像读 SCL 一样读 LAD：

- **新工具 `DescribeBlockLogic(softwarePath, blockPath)`**［L1，lite 档也含］：导出块后把每个 LAD 网络**重建成可读的能流表达式**——串联触点用 `·`、并联用 `+`、常闭显示 `/操作数`；线圈 `( )/(S)/(R)`、MOVE/比较/定时器盒带管脚操作数；SCL/STL 网络内联成代码。**关键**：自动标注接到字面常量的触点——`⟨恒断·禁用本行⟩`(常开触点接 FALSE=永久断开、静默禁用整行，肉眼几乎看不出)、`⟨恒通⟩`（恒导通旁路），比手工追 FlgNet XML 更直观。
  - 对 5T车 原生 `Auto_SpeedSet`(LAD+SCL+STL 混编)，可标出"初始化切换"里被恒 FALSE 触点禁用的 `0006/0007` 握手行(`TP(..).Q · 0 ⟨恒断·禁用本行⟩`)；复位行 `MOVE 16#04FE` 无禁用标记=有效。端到端(测试PLC/FC_Alarm)输出 `"DemoData.HighAlarm" ( ) ⇐ ("DemoData.Temp" >= "DemoData.Limit")`。
...
围绕「外部环境部署 + 弱模型 + 出错率」（lite 档位与 Doctor 此前只存在于 SKILL 文档、代码在迁仓时丢失，本版补齐）：
...
本轮全部围绕两个真实用户问题：①配置要人肉填 MCP 路径/博途路径/版本（issue #9）②非 Claude 的 AI 客户端调用时生成代码质量差、耗时长。
...
## [2.2.3] - 2026-06-15 - 弱模型健壮性：自愈连接 + 可恢复错误指引 + 操作规则

> 注：v2.2.2 版本号已被既有的「S7DCL skill 文档」Release 占用，故本代码版本顺延为 2.2.3。

针对"搭载 MCP 后较弱的 AI 模型仍会出 bug"的反馈，从源头降低出错与卡死，**工具数不变（189）**：
